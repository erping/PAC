...
<% 
	...
	String SQL2 = "select * from "
			+"(select * from (select* from daisuifang where time='6' and (complete='0' or complete='1') and shuming='"+shuming+"' and begin<='"+curTime.toString()+"' and end >='"+curTime.toString()+"'  order by gaowei asc,luoshi desc) as a "
			+" union all " 
			+"select * from (select * from daisuifang where time='6' and (complete='0' or complete='1') and shuming!='"+shuming+"' and begin<='"+curTime.toString()+"' and end >='"+curTime.toString()+"' order by gaowei asc,luoshi desc) as b)as c";
...
<table border="0" align="center" cellpadding="0" cellspacing="0" >
	<tr>
    	<td align="center" valign="top">    
			<div class="title">
    			<img width="900" height="42" src="Images/liugeyuedaisuifang.jpg"/>
			</div>
	...
	if(CurrPage2==CountPage2){
		SQL = "select * from "
				+"(select * from (select* from daisuifang where time='6' and (complete='0' or complete='1') and shuming='"+shuming+"' and begin<='"+curTime.toString()+"' and end >='"+curTime.toString()+"'  order by liudate asc,gaowei desc,luoshi asc) as a "
				+" union all " 
				+"select * from (select * from daisuifang where time='6' and (complete='0' or complete='1') and shuming!='"+shuming+"' and begin<='"+curTime.toString()+"' and end >='"+curTime.toString()+"' order by liudate asc,gaowei desc,luoshi asc) as b)as c" 
	...
	while(Rs.next()){ %>
		<tr class="a1" height="25px">
			...
			<td nowrap align="center">
			<% 
				String sql = "select * from zhongyuan where num='"+Rs.getString("num")+"'";
				ResultSet rs_c3 = selectall.executeQuery(sql);
				if(rs_c3.next()){%>
					<%=rs_c3.getString("person")%>
				<%} %>
			</td>
			<td nowrap align="center">
			...
			<td nowrap><a href="zhongyuan6.jsp?num=<%=Rs.getString("num")%>&shuming=<%=shuming%>&method=suifang" target=_parent><img width="55" height="20" src="Images/suifang.jpg"/></a></td>
	...
	<% } %>
		...
		<tr class="a1">
			<td colspan="9" align="center">
				<div align="center">
				<a href="daisuifang6.jsp?shuming=<%=shuming%>"><img width="100" height="20" src="Images/top.jpg"/></a>
				<% if (CurrPage2 > 1) { %>
				<a href="daisuifang6.jsp?Page=<%=CurrPage2 - 1%>&shuming=<%=shuming%>"><img width="100" height="20" src="Images/front.jpg"/></a>
				<% } %>
				<% if (CurrPage2 < CountPage2) { %>
				<a href="daisuifang6.jsp?Page=<%=CurrPage2 + 1%>&shuming=<%=shuming%>"><img width="100" height="20" src="Images/next.jpg"/></a>
				<% } %>
				<a href="daisuifang6.jsp?Page=<%=CountPage2%>&shuming=<%=shuming%>"><img width="100" height="20" src="Images/last.jpg"/></a>
				</div>
			</td>
		</tr>
 	</table>
