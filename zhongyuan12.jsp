...
function do_zhongyuan12(){
	...
	if(adviseform.name.value!="" && adviseform.num.value!=""){
		...
		url="do_zhongyuan12.jsp?name="+name+"&date="+date+"&num="+num+"&tel="+tel
...
function do_back(method){
	...
	if(method=="patient")
		...
		url = "daisuifang_n12.jsp?shuming=<%=request.getParameter("shuming")%>";
	window.location.href = url;
}
</script>
...
<form action="javascript:void(0)" name="adviseform" id="Form1" method="post" accept-charset="utf-8">
  <table class="b-content">
    <tr  id="r_table">
			<td  colspan="2">
    			<img width="725" height="45" src="Images/shiergeyuesuifang.jpg"/>
			</td>
		</tr>
    	<tr  id="r_table">
    	...
    	<%
    	String num,name,tel,nibiyun,biyun,liudate,COCshijian,COCchufang,IUDfangzhi,gaowei="";
	...
	<%
    ResultSet Rs3;
    String SQL3 = "select * from zhongyuan where num="+num.toString();
	...
	ResultSet Rs6;
    String SQL6 = "select * from zhongyuan6 where num="+num.toString();
	Rs6 = dao.executeQuery(SQL6);
	if(Rs6.getRow()!= 0&&Rs6.next()){
    ...
    <%if(count!=0) {%>
    <tr  id="r_table" >
      <td class="r_cell_1" colspan="2"><div>&nbsp;&nbsp;此患者在流产后6个月内曾有<%=count %>次妊娠</div></td>
    </tr>
    ...
    <tr  id="r_table" class="disrenshen" style="display:none">
      ...
      <td>
        <select name="renshen">
          	<option value="1">流产后3个月内</option>
          	<option value="2">流产后6个月内</option>
          	<option value="3">流产后1年内</option>
        </select>&nbsp;</td>
    </tr>
    <tr  id="r_table" class="disrenshen" style="display:none">
	...
	<tr  id="r_table">
	 ...
	 <tr  id="r_table">
       ...
       <td align="right">
	   <input type="image" id="save1" src="Images/save.jpg" onClick="do_zhongyuan12()"/>
       </td>
    </tr>
  </table>
 </form>
